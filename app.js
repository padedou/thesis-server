...
app.post("/sendRankings", /*bodyParser.json({limit: "1024mb"}),*/ function(req, res){
	...
	fs.writeFile("./public/servefiles/mpd/" + uuid + ".mpd", mpdString, "utf8", (err) => {
		if(err){
			res.status(500).send("Could not create the MPD file");
		}

		fs.writeFile("./public/servefiles/models/" + uuid + ".json", JSON.stringify(mpdModel.getLODs()), "utf8", (err) => {
			if(err){
			...
			*/
			...
			console.log("done");
			//res.render(path.join(__dirname, "./directions.pug"), autogeneratedInput);
